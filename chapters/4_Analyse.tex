...
Sobald das Datenmodell einer Domain identifiziert und in Entities sowie Value Objects unterteilt wurde, müssen den Domains Aggregate Roots zugeordnet werden \cite{DDD:Holmstrom:2019}. Wie bereits in Abschnitt \ref{sec:basics:ddd:mdd:aggregate} erläutert können nur Entities die Rolle als Aggregate Root einnehmen.
...
Im ersten Schritt wurde, anhand der Formel \ref{align:analyse:metric:total}, die Wertigkeit aller Kriterien berechnet. Dafür werden die Gewichtungen aller benötigten Kriterien summiert. Dadurch erhält man die maximal mögliche Bewertung des Systems.
...
Als Nächstes wurde die Formel \ref{align:analyse:metric:fulfilled} verwendet, um die Summe der Gewichtungen aller erfüllten Kriterien zu erhalten. Dieser Wert symbolisiert den tatsächlichen Zustand des Systems.
...
Um die Metrik aus \ref{align:analyse:metric} besser einzuordnen, wird dem System anhand der Tabelle \ref{tab:analyse:metric} eine Note gegeben. Die Grenzwerte der einzelnen Noten sind von SonarQube inspiriert und orientieren sich an dem Maintainability Rating \cite{Related:SonarSource:2019}. Es wurde diese Metrik ausgewählt da sie einen ähnlichen Sachverhalt beschreibt. 
...
Anschließend wird die Gesamtbewertung in dem Format aus Listing \ref{lst:analyse:metric:output}, mit zusätzlichen Informationen, zurückgegeben.