...
\label{ch:analyse}
Diese Kapitel behandelt die Umsetzung der Architekturanalyse in Verbindung mit der Entwicklung einer Metrik zur Architekturbewertung. Im ersten Schritt wird darauf eingegangen anhand welcher Merkmale Domain-Driven Design Bausteine erkannt werden können. Anschießend werden die Bewertungskriterien für die eingelesenen Bausteine vorgestellt. Zum Schluss werden die einzelnen Bewertungen zu einer Metrik zusammengefasst, um den Zustand des eingelesenen Systems zu bewerten.
...
\begin{lstlisting}[caption={Ausschnitt der Bewertung eines Artefakts},captionpos=b,label=lst:analyse:rating:issue:output, language=json]
{
    "DDD": "VALUE_OBJECT",
    "fitness": 50,
    "domain": "issue",
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{gfx/OOD_Quality_Metric.pdf}
    \caption{Zusammenhang zwischen den Object-Oriented Design Metriken}
    \label{fig:analyse:metric:ood}
\end{figure}

Laut \cite{Metric:Martin:1994} sollte die Abstraktheit und die Insabilität eines Moduls ausgeglichen sein. In der Abbildung \ref{fig:analyse:metric:ood} \cite{Metric:Martin:1994} ist dieses ausgeglichene Verhältnis als Diagonale dargestellt. Diese Diagonale wird auch Main Sequence gennant. In einem guten Object-Oriented Design sollten alle Packages auf der Main Sequence liegen.
...
\begin{lstlisting}[caption={Ausschnitt der Object-Oriented Design Bewertung eines Systems},captionpos=b,label=lst:analyse:metric:ood:output, language=json]
"OOD": {
    "distance": {
        "avg": 0.39,
        "min": 0.17,
...
\begin{lstlisting}[caption={Ausschnitt der Domain-Driven Design Bewertung eines Systems},captionpos=b,label=lst:analyse:metric:ddd:output, language=json]
"DDD": {
    "artifact": {
