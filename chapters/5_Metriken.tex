...
\label{ch:rating}
Diese Kapitel erläutert die Architekturbewertung eines Java Systems. Zuerst werden die Bewertungskriterien für die eingelesenen Bausteine vorgestellt. Anschießend werden die einzelnen Bewertungen zu einer Metrik zusammengefasst, um den Zustand des eingelesenen Systems zu bewerten.


\section{Bewertung der Domain-Driven Design Artefakte}
\label{sec:rating}
In diesem Abschnitt wird darauf eingegangen anhand von welchen Bewertungskriterium die Artefakte bewertet werden. Ein Bewertungskriterium besteht einer Beschreibung was erfüllt sein muss, um das Kriterium zu erfüllen und der Relevanz des Kriteriums. Die Relevanz spiegelt die Gewichtung des Bewertungskriteriums wieder.
...
Kriterien welche hingegen als \texttt{CRITICAL} oder \texttt{BLOCKER} gekennzeichnet wurden, sollten zwingend erfüllt sein, da sonst Grundprinzipien vom Domain-Driven Design verletzt werden. Deshalb werden diese Kriterien besonders stark gewichtet.
...
\begin{table}[htbp] 
    ...
    \begin{tabular}{|m{0.8\textwidth}|c|}   \hline
        ...
        Modul: \texttt{domain}                                              & \texttt{MINOR}    \\ \hline
...
Das letzte Kriterium des Domain-Moduls, aus der Tabelle \ref{tab:rating:modul:domain:criteria}, ist gleichzeitig das wichtigste. Denn jedes Domain-Modul symbolisiert eine Domain und muss daher genau eine Aggregate Root beinhalten. Da dieses Kriterium für die korrekte Umsetzung von Domain-Driven Design notwendig ist, wurde es als \texttt{CRITICAL} eingestuft.

\begin{table}[htbp] 
    \centering
    \begin{tabular}{|m{0.8\textwidth}|c|}   \hline
        \textbf{Kriterien}                                                  & \textbf{Relevanz} \\ \hline\hline
        Modul: \texttt{application}                                         & \texttt{MINOR}    \\ \hline
...
\begin{table}[htbp] 
    ...
    \begin{tabular}{|m{0.8\textwidth}|c|}   \hline
        \textbf{Kriterien}                                                  & \textbf{Relevanz} \\ \hline\hline
        Modul: \texttt{infrastructure}                                      & \texttt{MINOR}    \\ \hline
...
\begin{table}[htbp] 
    ...
    \begin{tabular}{|m{0.8\textwidth}|c|}   \hline
        \textbf{Kriterien}                                                  & \textbf{Relevanz} \\ \hline\hline
        Modul: \texttt{domain.<domain>.model}                               & \texttt{MINOR}    \\ \hline
...
\begin{table}[htbp] 
    ...
    \begin{tabular}{|m{0.8\textwidth}|c|}   \hline
        ...
        Setter für jedes Attribut                                           & \texttt{MINOR}    \\ \hline
        Setter sind frei von Seiteneffekten                                  & \texttt{CRITICAL} \\ \hline
    \end{tabular}
    \caption{Kriterien zur Bewertung von Value Objects}
    \label{tab:rating:artifakt:value_object:criteria}
\end{table}

Die Kriterien von Value Objects aus Tabelle \ref{tab:rating:artifakt:value_object:criteria} sind den Kriterien der Entities sehr ähnlich, unterscheiden sich aber teilweise deutlich. Das erste unterschiedliche Kriterium steht in der zweiten Zeile. Das Kriterium sagt aus, dass Value Objects nur Attribute von anderen Value Objects oder Java Standard Datentypen besitzen darf.

Das erste Kriterium, von Value Objects, welches sich grundlegend unterscheidet, steht in der dritten Zeile der Tabelle \ref{tab:rating:artifakt:value_object:criteria}. Wie bereits in Abschnitte \ref{sec:basics:ddd:mdd:value_object} beschrieben, stellt die Existenz einer Identität eine Architekturverletzung dar. Deshalb wurde dieses Kriterium als \texttt{BLOCKER} eingestuft.
...
Auch die Kriterien von Aggregate Roots, in der Tabelle \ref{tab:rating:artifakt:aggregate:criteria}, sind den Kriterien von Entities sehr ähnlich. Da Aggregate Roots, wie bereits in Abschnitt \ref{sec:basics:ddd:mdd:aggregate} beschrieben, nur spezielle Entities sind, sind die ersten sechs Kriterien identisch.

Zusätzlich zu den ersten sechs Kriterien besitzt die Aggregate Root drei weitere. Da eine Aggregate Root ihre zugeordnete Domain repräsentiert, werden zur Verwaltung drei Komponenten benötigt. Nämlich ein Repository, eine Factory und mindestens einen Domain Service \cite{DDD:Vernon:2013}. Diese drei Kriterien wurden, aufgrund ihrer Relevanz für die Architektur, als \texttt{MAJOR} eingestuft.
...
\begin{table}[htbp] 
    ...
    \begin{tabular}{|m{0.8\textwidth}|c|}   \hline
        ...
        Interface-Modul: \texttt{domain.<domain>.model}                     & \texttt{MINOR}    \\ \hline
        Klassen-Modul: \texttt{domain.<domain>.model.impl}                  & \texttt{MINOR}    \\ \hline
...
Die ersten fünf Kriterien von Repositories, in der Tabelle \ref{tab:rating:artifakt:repository:criteria} sind gleich aufgebaut wie im vorherigen Abschnitt \ref{sec:rating:artifakt:factory}.
...
In der zweiten Kategorie sind Methoden, welche nützlich sind aber nicht so relevant wie die Methoden aus der ersten Kategorie. Dazu gehören Methoden zum Aktualisieren und Überprüfen der Existenz einer Entity. Aufgrund ihrer geringeren Relevanz wurden diese Methoden als \texttt{MINOR} bewertet.
...
Das erste Kriterium, aus der Tabelle \ref{tab:rating:artifakt:event:criteria}, von Domain Events verhält sich analog zu den bereits behandelten Komponenten. 
...
\begin{table}[htbp] 
    ...
    \begin{tabular}{|m{0.8\textwidth}|c|}   \hline
        ...
        Application Service Modul: \texttt{application}                     & \texttt{MINOR}    \\ \hline
        Domain Service Modul: \texttt{application.<domain>}                 & \texttt{MINOR}    \\ \hline
    \end{tabular}
...
Die Application Services werden anhand von zwei Kriterien bewertet, siehe Tabelle \ref{tab:rating:artifakt:service:criteria}. Der Name des Application Service sollte das Wort \glqq Application\grqq{} beinhalten. Dieses Kriterium ist mit \texttt{INFO} gewichtet. Außerdem muss der Application Service dem Modul \texttt{application} zugeordnet sein. Die Zuordnung zu dem speziellen Modul ist als \texttt{MINOR} eingestuft.
...
Listing \ref{lst:rating:issue:output} zeigt einen Ausschnitt des Rückgabewertes eines bewerteten Artefakts. Man kann daran deutlich erkennen, welche Kriterien nicht eingehalten wurden und woran es gelegen hat.
...
In diesem Abschnitt werden verschiedene Metriken zur Bewertung von Object-Oriented Design behandelt. Die verwendeten Metriken stammen aus dem Artikel \glqq OO Design Quality Metrics\grqq{} \cite{Metric:Martin:1994}. Die Metriken beschreiben die Zustände der einzelnen Module eines Systems. Dabei wird die Abstraktheit sowie die Stabilität der Module gemessen.
...
Die Abstraktheit eines Moduls ist das Verhältnis von abstrakten Artefakten und allen anderen Artefakten \cite{Metric:Martin:1994}. Sie kann durch die Formel \ref{align:analyse:metric:ood:abstractness} berechnet werden. Der Wertebereich der Abstraktheit beträgt [0,1].

Um die Stabilität eines Moduls zu bestimmen, müssen die Abhängigkeiten des Moduls betrachtet werden. Dabei muss zwischen Afferent und Efferent Couplings unterschieden werden. Unter Afferent Couplings versteht man die Anzahl der Artefakte außerhalb dieses Packages, welche von diesem Package abhängig sind \cite{Metric:Martin:1994}. Efferent Couplings ist das Gegenteil von Afferent Couplings. Efferent Couplings ist die Anzahl der Klassen innerhalb dieses Packages, welche von einem anderem Package abhängen.
...
Ein Modul gilt als instabil, wenn es mehr Efferent als Afferent Couplings besitzt \cite{Metric:Martin:1994}. Anhand der Formel \ref{align:analyse:metric:ood:instability} kann die Instabiliät berechnet werden. Auch hier ist der Wertebereich [0,1].

\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{gfx/metric-temp-ood.pdf}
    \caption{Zusammenhang zwischen den Object-Oriented Design Metriken}
    \label{fig:analyse:metric:ood}
\end{figure}

Laut \cite{Metric:Martin:1994} sollte die Abstraktheit und die Insabilität eines Moduls ausgeglichen sein. In der Abbildung \ref{fig:analyse:metric:ood} ist dieses ausgeglichene Verhältnis als Diagonale dargestellt. Diese Diagonale wird auch Main Sequence gennant. In einem guten Object-Oriented Design sollten alle Packages auf der Main Sequence liegen.

\begin{equation}
    \text{Distanz} = | \text{Abstractness} + \text{Instability} - 1 | \label{align:analyse:metric:ood:distance}
\end{equation}

Um die Abweichung von der Main Sequence zu ermitteln, kann die Distance zu der Main Sequence mit der Formel \ref{align:analyse:metric:ood:distance} ermittelt werden.
...
Die in diesem Abschnitt beschriebenen Metriken werden für jedes Module berechnet und in dem Format aus Listing \ref{lst:analyse:metric:ood:output} zurückgegeben.
...
Im ersten Schritt wurde, anhand der Formel \ref{align:analyse:metric:ddd:total}, die Wertigkeit aller Kriterien berechnet. Dafür werden die Gewichtungen aller benötigten Kriterien summiert. Dadurch erhält man die maximal mögliche Bewertung des Systems.
...
Als Nächstes wurde die Formel \ref{align:analyse:metric:ddd:fulfilled} verwendet, um die Summe der Gewichtungen aller erfüllten Kriterien zu erhalten. Dieser Wert symbolisiert den tatsächlichen Zustand des Systems.

\begin{equation}
    \text{Metrik} = \frac{\text{Erfüllte Kriterien} \cdot 100}{\text{Totale Kriterien}} \label{align:analyse:metric:ddd}
\end{equation}

Zur Berechnung der Gesamtbewertung werden die vorherigen Ergebnisse mit der Formel \ref{align:analyse:metric:ddd} miteinander verrechnet. Dabei erhält man eine Metrik, welche die prozentuale Erfüllung aller benötigten Kriterien, unter Einbeziehung ihrer Relevanz, aussagt.

\begin{table}[htbp] 
    \centering
    \begin{tabular}{|c|l|l|}   \hline
        \textbf{Note}   & \textbf{Beschreibung} & \textbf{Wertebereich}                 \\ \hline\hline
        \texttt{A}      & sehr gut              & $\text{Metrik } \geq 95.0$            \\ \hline
        \texttt{B}      & gut                   & $95.0 > \text{ Metrik } \geq 90.0$    \\ \hline
        \texttt{C}      & befriedigend          & $90.0 > \text{ Metrik } \geq 80.0$    \\ \hline
        \texttt{D}      & ausreichend           & $80.0 > \text{ Metrik } \geq 50.0$    \\ \hline
        \texttt{E}      & mangelhaft            & $50.0 > \text{ Metrik } \geq 20.0$    \\ \hline
        \texttt{F}      & ungenügend            & $20.0 > \text{ Metrik } \geq 0.0$     \\ \hline
    \end{tabular}
    \caption{Benotung der Domain-Driven Design Metrik}
    \label{tab:analyse:metric:ddd}
\end{table}

Um die Metrik aus \ref{align:analyse:metric:ddd} besser einzuordnen, wird dem System anhand der Tabelle \ref{tab:analyse:metric:ddd} eine Note gegeben. Die Grenzwerte der einzelnen Noten sind von SonarQube inspiriert und orientieren sich an dem Maintainability Rating \cite{Metric:SonarSource:2019}. Es wurde diese Metrik ausgewählt da sie einen ähnlichen Sachverhalt beschreibt. 
...
Anschließend wird die Gesamtbewertung in dem Format aus Listing \ref{lst:analyse:metric:ddd:output}, mit zusätzlichen Informationen, zurückgegeben.