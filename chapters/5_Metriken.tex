...
Die Kriterien von Value Objects aus Tabelle \ref{tab:rating:artifakt:value_object:criteria} sind den Kriterien der Entities sehr ähnlich, unterscheiden sich aber teilweise deutlich. In der zweiten Zeile befindet sich eines der abweichenden Kriterien. Das Kriterium sagt aus, dass Value Objects nur Attribute von anderen Value Objects oder Java Standard Datentypen besitzen darf.

Das zweite Kriterium, von Value Objects, welches sich grundlegend unterscheidet, steht in der dritten Zeile der Tabelle \ref{tab:rating:artifakt:value_object:criteria}. Wie bereits in Abschnitte \ref{sec:basics:ddd:mdd:value_object} beschrieben, stellt die Existenz einer Identität eine Architekturverletzung dar. Deshalb wurde dieses Kriterium als \texttt{BLOCKER} eingestuft.
...
Bei den Settern hingegen muss beachtet werden, dass sie keine Seiteneffekte haben \cite{DDD:Evans:2003}. Dieses Kriterium wird erfüllt, wenn die Methode als \texttt{private} gekennzeichnet wurde oder wenn die Methode eine neue Instanz von dem Value Objects zurück gibt. In beiden Fällen ist gewährleistet, dass die Werte des Objects nicht verändert werden. Da die Unveränderbarkeit der Value Objects eine Grundvoraussetzung ist, wird dieses Kriterium als \texttt{CRITICAL} eingestuft.
...
Zusätzlich zu den ersten sechs Kriterien besitzt die Aggregate Root drei weitere. Da eine Aggregate Root ihre zugeordnete Domain repräsentiert, werden zur Verwaltung drei Komponenten benötigt. Nämlich ein Repository, eine Factory und mindestens einen Domain Service \cite{DDD:Vernon:2013}. Diese drei Kriterien werden, aufgrund ihrer Relevanz für die Architektur, als \texttt{MAJOR} eingestuft.
...
Da eine Factory, wie im Abschnitt \ref{sec:basics:ddd:mdd:factory} beschrieben, für die Erzeugung von Entities zuständig ist, benötigt sie eine Instanz des dazugehörigen Repositories, um die neue Entity zu persistieren. Das Erzeugen der Entity sollte, wie in der letzten Zeile der Tabelle \ref{tab:rating:artifakt:factory:criteria} beschrieben, mit der Methode \texttt{create()} geschehen. Die beiden letzten Kriterien werden mit einer Relevanz von \texttt{MAJOR} bewertet.
...
Laut Martin\cite{Metric:Martin:1994} sollte die Abstraktheit und die Insabilität eines Moduls ausgeglichen sein. In der Abbildung \ref{fig:analyse:metric:ood} ist dieses ausgeglichene Verhältnis als Diagonale dargestellt. Diese Diagonale wird auch Main Sequence gennant. In einem guten Object-Oriented Design sollten alle Module auf der Main Sequence liegen.

\begin{equation}
    \text{Distanz} = | \text{Abstractness} + \text{Instability} - 1 | \label{align:analyse:metric:ood:distance}
\end{equation}

Um die Abweichung von der Main Sequence zu ermitteln, kann die Distance zu der Main Sequence mit der Formel \ref{align:analyse:metric:ood:distance} ermittelt werden. Da ein Modul im besten Fall keine Abweichung von der Main Sequence haben sollte, ist der bestmögliche Wert für die Distanz \texttt{0}.
...
Im ersten Schritt wird, anhand der Formel \ref{align:analyse:metric:ddd:total}, die Wertigkeit aller Kriterien berechnet. Dafür werden die Gewichtungen aller benötigten Kriterien summiert. Dadurch erhält man die maximal mögliche Bewertung des Systems.
...
Als Nächstes wird die Formel \ref{align:analyse:metric:ddd:fulfilled} angewandt, um die Summe der Gewichtungen aller erfüllten Kriterien zu erhalten. Dieser Wert symbolisiert den tatsächlichen Zustand des Systems.

\begin{equation}
    \text{Metrik} = \frac{\text{Erfüllte Kriterien} \cdot 100}{\text{Totale Kriterien}} \label{align:analyse:metric:ddd}
\end{equation}

Zur Berechnung der Gesamtbewertung werden die vorherigen Ergebnisse in die Formel \ref{align:analyse:metric:ddd} eingesetzt. Dabei erhält man eine Metrik, welche die prozentuale Erfüllung aller benötigten Kriterien, unter Einbeziehung ihrer Relevanz, aussagt.
