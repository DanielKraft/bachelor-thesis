...
In diesem Abschnitt werden verschiedene Metriken zur Bewertung von Ob\-ject-Oriented Design behandelt. Die verwendeten Metriken stammen aus dem Artikel \glqq OO Design Quality Metrics\grqq{} \cite{Metric:Martin:1994}. Die Metriken beschreiben die Zustände der einzelnen Module eines Systems. Dabei wird die Abstraktheit sowie die Stabilität der Module gemessen.
...
Laut \cite{Metric:Martin:1994} sollte die Abstraktheit und die Insabilität eines Moduls ausgeglichen sein. In der Abbildung \ref{fig:analyse:metric:ood} ist dieses ausgeglichene Verhältnis als Diagonale dargestellt. Diese Diagonale wird auch Main Sequence gennant. In einem guten Object-Oriented Design sollten alle Module auf der Main Sequence liegen.

\begin{equation}
    \text{Distanz} = | \text{Abstractness} + \text{Instability} - 1 | \label{align:analyse:metric:ood:distance}
\end{equation}

Um die Abweichung von der Main Sequence zu ermitteln, kann die Distance zu der Main Sequence mit der Formel \ref{align:analyse:metric:ood:distance} ermittelt werden. Da die ein Modul im besten Fall keine Abweichung von der Main Sequence haben sollte, ist der bestmögliche Wert für die Distanz \texttt{0}.
