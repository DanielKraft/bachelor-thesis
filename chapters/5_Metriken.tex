...
Die Kriterien für Applikation- und Infrastruktur-Module, aus der \autoref{tab:rating:modul:applictaion:criteria} und der \autoref{tab:rating:modul:infrastructure:criteria}, sind den ersten zwei Kriterien des Domain-Moduls sehr ähnlich.
...
Die Application Services werden anhand von zwei Kriterien bewertet, siehe \autoref{tab:rating:artifakt:service:criteria}. Der Name des Application Service sollte das Wort \glqq Application\grqq{} beinhalten. Dieses Kriterium ist mit \texttt{INFO} gewichtet. Außerdem muss der Application Service dem Modul \texttt{application} zugeordnet sein. Die Zuordnung zu dem speziellen Modul ist als \texttt{MINOR} eingestuft.
...
\autoref{lst:rating:issue:output} zeigt einen Ausschnitt des Rückgabewertes eines bewerteten Artefakts. Man kann daran deutlich erkennen, welche Kriterien nicht eingehalten wurden und woran es gelegen hat.
...
Die in diesem Abschnitt beschriebenen Metriken werden für jedes Module berechnet und in dem Format aus \autoref{lst:analyse:metric:ood:output} zurückgegeben.
...
Im ersten Schritt wird, anhand der \autoref{align:analyse:metric:ddd:total}, die Wertigkeit aller Kriterien berechnet. Dafür werden die Gewichtungen aller benötigten Kriterien summiert. Dadurch erhält man die maximal mögliche Bewertung des Systems.
...
Zur Berechnung der Gesamtbewertung werden die vorherigen Ergebnisse in die \autoref{align:analyse:metric:ddd} eingesetzt. Dabei erhält man eine Metrik, welche die prozentuale Erfüllung aller benötigten Kriterien, unter Einbeziehung ihrer Relevanz, aussagt.
